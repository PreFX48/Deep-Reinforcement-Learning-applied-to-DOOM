...
class Worker(mp.Process):
    def __init__(self, global_net, opt, global_ep, global_ep_r, res_queue, name):
        ...
        self.game, self.possible_actions = create_environment(scenario=SCENARIO)
    ...
    def run(self):
        ...
        while self.global_ep.value < TOTAL_EPISODES:
            ...
            episode_length = 1
            losses = []
            state, last_frames = stack_frames(last_frames, get_state(self.game), is_new_episode=True, maxlen=STACK_SIZE)
            buffer_s, buffer_a, buffer_r = [], [], []
            total_reward = 0.
            while True:
                buffer_s.append(state)
                action = self.local_net.choose_action(state)
                r = self.game.make_action(self.possible_actions[action], FRAME_SKIP)
                ...
                if total_step % BATCH_SIZE == 0 or done:  # update global and assign to local net
                    ...
                    if done:  # done and print information
                        self.log_episode(total_reward, episode_length, np.array(losses).mean())
                        episode += 1
                        break
                total_step += 1
                episode_length += 1
    ...
    def log_episode(self, episode_reward, episode_length, mean_loss):
        ...
        vars = {
            'Game/Kills': self.game.get_game_variable(KILLCOUNT),
            'Game/Ammo': self.game.get_game_variable(AMMO2),
